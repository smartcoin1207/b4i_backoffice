...
// SELECT startups.id as startup_id, startups.startup_name, co_founders.* FROM co_founders LEFT JOIN startups ON co_founders.startup_ids LIKE CONCAT('%{', startups.id, '}%')

if( !empty($_GET) ) {
	$q = "";
	if( !empty($_GET["q"]) ) {
		$q = $_GET["q"];
		$q = "AND (co_founders.name LIKE '%$q%' OR co_founders.surname LIKE '%$q%' OR co_founders.nationality LIKE '%$q%' OR co_founders.university LIKE '%$q%' OR co_founders.degree LIKE '%$q%')";
	}
	$date_start = "";
	if( !empty($_GET["date_start"]) ) {
		$date_start = "AND co_founders.created_on >= '".$_GET["date_start"]."'";
	}
	$date_end = "";
	if( !empty($_GET["date_end"]) ) {
		$date_end = "AND co_founders.created_on <= '".$_GET["date_end"]."' + INTERVAL 1 DAY";
	}
	$call_name = "";
	if( !empty($_GET["call_name"]) ) {
		$call_name = "AND co_founders.call_name = '".$_GET["call_name"]."'";
	}
	/*
	$application_request = "";
	...
	*/
	...
	if( !empty($_GET["country"]) ) {
		$country = "AND co_founders.nationality = '".$_GET["country"]."'";
	}
	if( !empty($_GET["stranger"]) ) {
		$country = "AND co_founders.nationality <> 'IT'";
	}
	$gender = "";
	if( !empty($_GET["gender"]) ) {
		$gender = "AND co_founders.gender = '".$_GET["gender"]."'";
	}
	$university = "";
	if( !empty($_GET["university"]) ) {
		if( $_GET["university"]=="other" ) {
			$university = "AND co_founders.university NOT IN ('".implode("', '", $universities)."')";
		} else {
			$university = "AND co_founders.university = '".$_GET["university"]."'";
		}
	}
	$alumnus = "";
	if( !empty($_GET["alumnus"]) ) {
		$alumnus = "AND co_founders.alumnus = 'Yes'";
	}
	$iit_researcher = "";
	if( !empty($_GET["iit_researcher"]) ) {
		$iit_researcher = "AND co_founders.iit_researcher = 'Yes'";
	}
	$developer = "";
	if( !empty($_GET["developer"]) ) {
		$developer = "AND co_founders.developer = 'Yes'";
	}
	$status = "co_founders.status<>'-1'";
	if( !empty($_GET["status_active"]) && !empty($_GET["status_inactive"]) ) {
		$status = "(co_founders.status = '1' OR co_founders.status = '0')";
	}
	if( !empty($_GET["status_active"]) && empty($_GET["status_inactive"]) ) {
		$status = "co_founders.status = '1'";
	}
	if( empty($_GET["status_active"]) && !empty($_GET["status_inactive"]) ) {
		$status = "co_founders.status = '0'";
	}
	$grant_application = "";
	if( !empty($_GET["grant_application"]) ) {
		$grant_application = "AND startups.grant_application = '".$_GET["grant_application"]."'";
	}
	//$query = "SELECT co_founders.* FROM co_founders WHERE $status $q $date_start $date_end $call_name $country $gender $university $alumnus $iit_researcher $developer ORDER BY created_on DESC";
	...
	if( !empty($_GET["action"]) && $_GET["action"]=="export" ) {
		...
		$file = "co_founders.csv";
		include("_export_founders.php");
	}
	$co_founders = DB::query($query);
} else {
	$query = "SELECT * FROM co_founders ORDER BY created_on DESC";
	$co_founders = DB::query($query);
}

$calls = DB::query("SELECT DISTINCT(call_name) FROM co_founders WHERE call_name<>'' ORDER BY call_name ASC");
//$universities = DB::query("SELECT DISTINCT(university) FROM co_founders WHERE university<>'' ORDER BY university ASC");
				...
				<div class="container-fluid">
					...
					<div class="card shadow mb-4">
						<div class="card-header d-sm-flex align-items-center justify-content-between py-3">
							<h6 class="m-0 font-weight-bold text-primary d-inline-block">Co-founders</h6>
							<div>
						...
						<form id="search_form" class="card-header py-4" method="get" style="min-width: 1120px; <?php if( empty($_GET) ) echo "display: none;";?>">
							...
							<div class="text-right">
								<a class="btn btn-sm btn-light" href="<?php echo BASEURL;?>backoffice/co_founders/">Reset</a>
							...
							<form class="table-responsive" id="founders-list">
								<table class="table table-bordered" id="dataTable" width="100%" cellspacing="0" data-order='[[ 7, "desc" ]]'>
									...
									foreach($co_founders as $co_founder) {
										$date = date("d/m/Y", strtotime($co_founder["created_on"]));
										if( $co_founder["status"]=='0' ) $style = 'style="opacity:.5"';
										else $style = '';
									?>
										<tr <?php echo $style;?>>
											<td>
												<input type="checkbox" name="status[]" value="<?php echo $co_founder["id"];?>" data-status="<?php echo $co_founder["status"];?>">
											</td>
											<td>
											...
											<td data-order="<?php echo $co_founder["nationality"];?>"><img src="<?php echo BASEURL."images/flags/".strtolower($co_founder["nationality"]);?>.svg" class="flag" title="<?php echo $co_founder["nationality"];?>"></td>
											<td><?php echo htmlentities($co_founder["university"]);?></td>
											<td><?php echo htmlentities($co_founder["degree"]);?></td>
											<td><?php echo htmlentities($co_founder["alumnus"]);?></td>
											<td><?php echo htmlentities($co_founder["iit_researcher"]);?></td>
											<td data-order="<?php echo $co_founder["created_on"];?>"><?php echo $date;?></td>
										</tr>
									<?php
									}
									?>
