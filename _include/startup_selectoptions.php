<?php
    ...
    SELECT * 
    FROM startups s
    WHERE s.granted_on IS NOT NULL AND s.`status` = 1
    AND (
        s.startup_name IN (
            SELECT startup_name
            FROM startups WHERE s.grant_application = 'Acceleration program' AND granted_on IS NOT NULL AND s.`status` = 1
        ...
        s.startup_name IN (
            SELECT startup_name
            FROM startups WHERE granted_on IS NOT NULL AND s.`status` = 1 
            GROUP BY startup_name
            HAVING COUNT(*) = 1
        )
    )
